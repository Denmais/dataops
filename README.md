...
▎Загрузка изображения (создание задания)

• POST /api/v1/jobs
  – multipart/form-data поле file
  – Ответ: job_id, status, ссылки на статус/результат

▎Получение статуса и результата

• GET /api/v1/jobs/{job_id}
  – Возвращает:
    ▪️ status=processing → UI показывает “Идёт распознавание…”
    ▪️ status=done → prediction + extra_info
    ▪️ status=failed → error_message + “попробуйте снова”

▎История загрузок пользователя

• GET /api/v1/jobs?limit=20&offset=0
  – Возвращает список работ пользователя (включая мини-инфо + ссылки)

▎Просмотр конкретной картинки (опционально)

• GET /api/v1/jobs/{job_id}/image
  – Отдаёт файл (или redirect на S3 pre-signed URL)

▎“Поделиться результатом”

• POST /api/v1/jobs/{job_id}/share
  – Создаёт публичный токен/ссылку
• GET /api/v1/share/{token}
  – Публичное получение результата (без авторизации), чтобы можно было шарить в соцсети как URL.
